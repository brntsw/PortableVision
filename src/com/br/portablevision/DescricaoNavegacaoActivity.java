...
public class DescricaoNavegacaoActivity extends Activity implements OnCompletionListener{
	...
	public void onCreate(Bundle savedInstanceState){
		...
		setContentView(R.layout.descricao_navegacao);
		...
		if(tipoNavegacao.equals("voz")){
	        ...
	        if(!jaFalouDescricao){
				...
				try {
					asset = getAssets().openFd("fale_novamente.mp3"); //Substituir pelo áudio da descrição navegação
					mp.setDataSource(asset.getFileDescriptor(), asset.getStartOffset(), asset.getLength());
					asset.close();
					mp.prepare();
					mp.start();
					
					mp.setOnCompletionListener(DescricaoNavegacaoActivity.this);
				} catch (IOException e) {
					e.printStackTrace();
				}
	        }
			...
			btnVoltar.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					finalizaActivity(DescricaoNavegacaoActivity.this);
				}
			});
		}
	}
